...
const config = {
    ...
    lightMinifyOptions: {
        compress: {
            drop_console: false, // Оставляем console.log для разработки
            drop_debugger: false, // Оставляем debugger для разработки
            passes: 1, // Минимальное количество проходов
            dead_code: true, // Удаляем мертвый код
            unused: false, // Не удаляем неиспользуемые переменные (может быть полезно для отладки)
            evaluate: true, // Вычисляем константные выражения
            join_vars: true, // Объединяем объявления переменных
            reduce_vars: false, // Не заменяем переменные константами (для читаемости)
